...
class Executor(ABC):
    ...
    @abstractmethod
    async def run_tool(self, session: ExecutionSession, tool, record, context,
                       cancellation_token: CancellationToken | None = None):
        """Invoke a native CoreTool in this provider; never fall back to host."""

    @abstractmethod
    async def execute_argv(
        self, session: ExecutionSession, argv: list[str], *, stdin: str | None = None,
        timeout: float = 60, cancellation_token: CancellationToken | None = None,
    ) -> ExecutionResult:
        """Execute an argument vector without shell interpolation, with bounded output."""

    @abstractmethod
    async def connect(
        self, workspace: Workspace, user_id: str, conversation_id: str,
    ) -> ExecutionSession:
        """Create/attach a session and mount or expose the user's workspace."""

    @abstractmethod
    async def execute(
        self, session: ExecutionSession, command: str, *, timeout: float = 60,
        cancellation_token: CancellationToken | None = None,
    ) -> ExecutionResult:
