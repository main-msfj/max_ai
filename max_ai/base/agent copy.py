...
class Agent(ComponentBase[BaseModel], ABC):
    ...
    async def _maintain_memory_after_compaction(
        ...
        *,
        ...
        if memory is None or not result.changed or not result.old_messages:
            return

        try:
            await memory._ensure_connected()
            facts = await memory.list_facts()
            task = self._memory_maintenance_task(facts, result.old_messages)
    ...
    def _memory_maintenance_task(
        ...
        return (
            ...
            "Review every stored memory fact and the conversation messages that "
            "are about to be compacted. Return only structured output.\n\n"
            "Rules:\n"
