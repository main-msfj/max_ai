...
class ToolExecutor:
    ...
    async def _invoke_tool(
        ...
        tool_ctx = ToolContext(
            run_id=ctx.run_id,
            session_id=ctx.runtime_state.session_id or "",
        )
    ...
    async def _yield_failure(
        ...
        """Emit a failure ``ToolMessage`` + ``ToolCallResponseEvent`` without running the tool.

        Used for cases where execution never happens: tool not found,
        validation failed, rejected by user, runner crashed, etc. The
        record is NOT marked CONSUMED — the executor only consumes
        records that genuinely passed through ``start_execution`` and
        ``mark_consumed`` (or stale failure handling).
        """
