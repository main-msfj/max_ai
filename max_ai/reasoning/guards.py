...
class RepetitionGuard(LoopGuard):
    ...
    def after_tool_round(self, ctx, state, guard_ctx):
        ...
        for tc in assistant.tool_calls:
            ...
            key = hashlib.sha256(
                f"{tc.tool_name}:{canonical}".encode()
            ).hexdigest()
...
class BudgetGuard(LoopGuard):
    ...
    how many iterations remain and to consolidate.
    """
    ...
    def after_tool_round(self, ctx, state, guard_ctx):
        ...
        return (
            f"You have {remaining} reasoning iteration(s) left in this turn. "
            "Consolidate what you have learned and produce your final answer "
            "for the user before the budget runs out."
        )
...
class PlanCompletionGuard(LoopGuard):
    ...
    def on_final_answer(self, ctx, state, guard_ctx):
        ...
        unfinished = [
            s for s in plan.steps if s.status not in ("done", "failed")
        ]
        listing = "; ".join(
            f"{s.id}. {s.description} [{s.status}]" for s in unfinished
        )
