...
_RESERVED_ROOT_DIRS = {"tools", "skills", "artifacts", "scratchpad", "conversation"}
# Hidden from root listings (list_directory/find_files at the user root).
# conversation stays visible so find_files/search_text keep descending into
# it — otherwise "search across all conversations" silently stops working.
_HIDDEN_ROOT_DIRS = _RESERVED_ROOT_DIRS - {"skills", "conversation"}
# Blocked from explicit path resolution too. scratchpad/conversation are
# hidden-or-visible but still reachable by name (like skills); tools/artifacts
# stay fully off-limits.
_BLOCKED_PATH_ROOTS = _RESERVED_ROOT_DIRS - {"skills", "scratchpad", "conversation"}
...
class UserFileSystem:
    ...
    def conversation_root(self, user_id: str, session_id: str) -> Path:
        ...
        user_fd = self._open_user_fd(user, create=True)
        try:
            parent_fd = self._open_dir_at(user_fd, "conversation", create=True)
            try:
                conversation_fd = self._open_dir_at(parent_fd, session, create=True)
                os.close(conversation_fd)
            finally:
                os.close(parent_fd)
        finally:
            os.close(user_fd)
        return self.root / user / "conversation" / session
    ...
    def create_text_file(
        ...
        target_parts = ("conversation", session, *parts)
        lock = _path_lock(
    ...
    def _conversation_file_parts(cls, path: str) -> tuple[str, ...]:
        ...
        if len(parts) < 3 or parts[0] != "conversation":
            raise ValueError("path must be a conversation/<id>/file path")
        cls._safe_session_id(parts[1])
        return parts
