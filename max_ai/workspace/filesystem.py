...
_RESERVED_ROOT_DIRS = {"tools", "skills", "artifacts", "scratchpad"}
_HIDDEN_ROOT_DIRS = _RESERVED_ROOT_DIRS - {"skills"}
...
class UserFileSystem:
    """Access ``root/user/conversation`` with user-wide read scope."""
    ...
    def conversation_root(self, user_id: str, session_id: str) -> Path:
        """Validate and securely materialize one conversation directory."""
        user = self._safe_id(user_id, "user_id")
        session = self._safe_session_id(session_id)
        user_fd = self._open_user_fd(user, create=True)
        try:
            conversation_fd = self._open_dir_at(user_fd, session, create=True)
    ...
    def scan_files(
        ...
        def walk(directory_fd: int, base: tuple[str, ...], depth: int, user_root: bool) -> None:
            ...
            with os.scandir(directory_fd) as entries:
                for entry in entries:
                    ...
                    if stat.S_ISDIR(info.st_mode):
                        ...
                        files.append({"path": "/".join(child_parts), "bytes": info.st_size})
    ...
    def read_snapshot(self, user_id: str, path: str) -> bytes:
        ...
        parts = self._conversation_file_parts(path)
    ...
    def create_text_file(
        self, user_id: str, session_id: str, path: str, content: str
    ) -> dict[str, Any]:
        """Create a new text file directly under the current conversation."""
        session = self._safe_session_id(session_id)
        parts = self._write_parts(path)
        data = self._text_bytes(content)
        target_parts = (session, *parts)
        lock = _path_lock((str(self.root), self._safe_id(user_id, "user_id"), "/".join(target_parts)))
        with lock:
            ...
            try:
                ...
                try:
                    ...
                    raise FileExistsError("file already exists; write_file only creates new files") from None
                os.unlink(temp_name, dir_fd=parent_fd)
                temp_name = None
                os.fsync(parent_fd)
            finally:
    ...
    def create_directory(
        self, user_id: str, session_id: str, path: str
    ) -> dict[str, Any]:
        """Create a directory below the current conversation."""
        session = self._safe_session_id(session_id)
        parts = self._write_parts(path)
        target_parts = (session, *parts)
        display_path = "/".join(target_parts)
        lock = _path_lock((str(self.root), self._safe_id(user_id, "user_id"), display_path))
        with lock:
            parent_fd = self._open_parent_fd(user_id, target_parts, create=True)
            try:
                try:
    ...
    def delete_file(
        ...
        with lock:
            ...
            parent_fd = self._open_parent_fd(user, parts)
            try:
                current_stat = os.stat(parts[-1], dir_fd=parent_fd, follow_symlinks=False)
                self._require_regular_single_link(current_stat)
                if (
                    current_stat.st_dev != opened_stat.st_dev
                    or current_stat.st_ino != opened_stat.st_ino
    ...
    def file_info(self, user_id: str, path: str) -> dict[str, Any]:
        ...
        data, _ = self._read_all(user_id, parts)
        return {
            "path": "/".join(parts), "type": "file", "bytes": len(data),
            "sha256": hashlib.sha256(data).hexdigest(),
        }
    ...
    def _visible_parts(cls, path: str, *, allow_empty: bool = False) -> tuple[str, ...]:
        ...
        if parts and parts[0] in _HIDDEN_ROOT_DIRS:
    ...
    @classmethod
    def _conversation_file_parts(cls, path: str) -> tuple[str, ...]:
        parts = cls._write_parts(path)
        if len(parts) < 2:
            raise ValueError("path must be a conversation/file path")
        cls._safe_session_id(parts[0])
        return parts
    ...
    def _raise_safe_path_error(exc: OSError) -> None:
        if exc.errno in {errno.ELOOP, errno.ENOTDIR, errno.EISDIR}:
            raise ValueError("symlinks and non-directory path components are not accessible") from None
