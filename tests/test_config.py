...
def test_compaction_settings_defaults():
    ...
    assert settings.compaction_prompt_budget_tokens == 3000
    assert settings.compaction_summary_budget_tokens == 2000
    assert settings.compaction_safety_margin_ratio == 0.05
    assert settings.compaction_live_message_threshold == 0.4
    assert settings.compaction_live_message_budget_tokens == 2000
...
def test_compaction_settings_can_be_overridden_from_env(monkeypatch):
    ...
    monkeypatch.setenv("COMPACTION_LIVE_MESSAGE_BUDGET_TOKENS", "2200")
    ...
    assert settings.compaction_live_message_budget_tokens == 2200
