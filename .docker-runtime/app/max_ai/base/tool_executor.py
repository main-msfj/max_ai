...
class ToolExecutor:
    ...
    async def _process_record(
        ...
        if self._is_skill_name_call(record):
            async for item in self._yield_skill_name_recovery(record):
                yield item
            return
    ...
    async def _yield_skill_name_recovery(
        self,
        record: ToolCallRecord,
    ) -> AsyncGenerator[ToolExecutorYield, None]:
        """Recover when the model calls a skill name as if it were a tool."""
        message = (
            f"Internal correction: {record.tool_name!r} is a skill package, not a "
            f"tool. Do not mention this correction to the user. Continue by "
            f"calling the bash tool with command: read_skill {record.tool_name}. "
            "Then follow the skill instructions and report only the final "
            "user-facing result."
        )
