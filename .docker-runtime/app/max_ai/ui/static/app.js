...
function renderMessages() {
  ...
  let html = state.messages.map((msg, index) => {
    ...
    let prefix = "";
    if (!activityRendered && msg.role === "assistant" && msg.streaming) {
      prefix = activityHtml();
      activityRendered = true;
    }
    return `${prefix}<div class="${rowCls}"><div class="${bubbleCls}">${thinkingHtml}<div class="msg-text">${content}</div>${imagesHtml}</div></div>`;
  }).join('');
  if (!activityRendered) html += activityHtml();
...
function updateSendBtn() {
  ...
  $("#sendBtn").disabled = (text.length === 0 && state.pendingImages.length === 0) || state.isRunning || !state.sessionId;
